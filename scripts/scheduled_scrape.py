...
async def main(force_all: bool = False, dry_run: bool = False):
    ...
    # 2. Gmail LISTSERV (always, needs Ollama) — capped at 10 min so Instagram still runs
    if ollama_ok and not dry_run:
        try:
            results["gmail"] = await asyncio.wait_for(
                run_gmail(logger), timeout=600,  # 10 min hard cap
            )
        except asyncio.TimeoutError:
            logger.error("Gmail poll timed out after 10 min — moving on to Instagram")
            results["gmail"] = {"error": "timeout_10m"}
    elif not ollama_ok:
